        ...
        <div className="max-w-2xl mb-10">
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full h-[44px] bg-[#F8FAFC] dark:bg-gray-900 border border-[#E5E7EB] dark:border-gray-800 rounded-[10px] pl-10 pr-4 text-[14px] text-[#111827] dark:text-white placeholder-[#6B7280] dark:placeholder-gray-500 focus:outline-none focus:border-[#2563EB] dark:focus:border-blue-500 transition-colors"
            />
          </div>
        </div>
        ...
        <div className="space-y-3 max-w-3xl">
          {filteredFaqs.map((faq) => {
            ...
            return (
              <div
                key={faq.id}
                className="bg-white dark:bg-gray-900 border border-[#E5E7EB] dark:border-gray-800 rounded-[14px] overflow-hidden transition-colors"
              >
                <button
                  onClick={() => setOpenId(isOpen ? null : faq.id)}
                  className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-[#F8FAFC] dark:hover:bg-gray-800/60 transition-colors cursor-pointer"
                >
                  <span className="text-[16px] font-medium text-[#111827] dark:text-white pr-4">
                    {faq.question}
                  </span>
                  <ChevronDown
          ...
          {filteredFaqs.length === 0 && (
            <div className="py-8 text-[#6B7280] dark:text-gray-400 text-[14px]">
